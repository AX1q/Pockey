    ...
    for contour in contours:
        ...
        if len(approx) == 4:
            ...
            if 0.3 < aspect_ratio < 1.5:
                ...
                if not any(((abs(corner[0] - prev_corner[:, 0]) < 10) & (abs(corner[1] - prev_corner[:, 1]) < 10)).any() for prev_corner in detected_rectangles_corners for corner in corners):
                    ...
                    for corner in corners:
                        ...
                        text_x = x + 5 if x + 5 < image.shape[1] - 10 else x - 45
                        text_y = y - 5 if y - 5 > 10 else y + 25

                        cv2.putText(image, f"({x}, {y})", (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 100, 0), 2)
                    
    # 如果检测到了至少一个矩形
    if detected_rectangles_corners:
        # 获取最后一个矩形的四个角点
        corners = detected_rectangles_corners[-1]
